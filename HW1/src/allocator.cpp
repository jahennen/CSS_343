...
void* allocate(size_t request) {
	...
	while (current) {
	    ...
	    // no block big enough, throw error, die horribly
...
void deallocate(void * mem){
	...
	if (root + ARENA_SIZE <= mem || mem <= root+hdr || mem == NULL) {
		fprintf(stderr,"Trying to deallocate a ptr not belonging to arena OR ptr = NULL\n");
		return;
	}
